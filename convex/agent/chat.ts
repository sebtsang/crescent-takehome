 ...
 * a zero count and the dataset's date bounds, not because the model is clever.
...
export const ask = action({
  ...
  handler: async (ctx, { threadId }): Promise<{ ok: boolean; error?: string }> => {
    /**
     * The key lives on the CONVEX DEPLOYMENT, not in .env.local. This action
     * executes on Convex's servers, so `process.env` resolves against the
     * deployment environment -- a key in .env.local would never reach it.
     */
    ...
    if (!apiKey) {
      await ctx.runMutation(internal.chat.appendMessage, {
        ...
        content:
          ...
          'Note this goes on the deployment, not in .env.local -- the assistant ' +
          'runs in a Convex action, so it reads the deployment environment.',
      });
